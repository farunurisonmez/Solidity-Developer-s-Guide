...
contract Reserve is IERC721Receiver {
    ...
    function createNewListing(uint256 tokenId, uint256 price) public {
        require(
            address(this) == nftContract.ownerOf(tokenId),
            "Please transfer the NFT to this contract."
        );
    ...
    function randomBuy() public payable {
        ...
        for (uint256 i = 0; i < _tokenIDs.length; i++) {
            Listing storage listing = tokenIdToListing[
                listings[_tokenIDs[i]].tokenId
            ][getListingHistorySize(listings[_tokenIDs[i]].tokenId) - 1];
    ...
    function randomId() internal returns (uint256[] memory) {
        ...
        uint256 randomIndex = uint256(
            keccak256(
                abi.encodePacked(revealNonce, block.difficulty, msg.sender)
            )
        ) % 3;
    ...
    function closeListing(uint256 listingID) public {
        ...
        require(
            listings[listingID].sold == false,
            "This action cannot be performed on a sold listing."
        );
    ...
    function setListingPrice(uint256 listingID, uint256 newPrice) public {
        require(
            msg.sender == owner,
            "Only the contract owner can call this function."
        );
    ...
    function isListingOpenByTokenId(uint256 tokenId)
        public
        view
        returns (bool)
    {
    ...
    function isListingSoldByTokenId(uint256 tokenId)
        public
        view
        returns (bool)
    {
    ...
    function getListingSellerByTokenId(uint256 tokenId)
        public
        view
        returns (address)
    {
    ...
    function getListingPriceByTokenId(uint256 tokenId)
        public
        view
        returns (uint256)
    {
    ...
    function setListingPriceByTokenId(uint256 tokenId, uint256 newPrice)
        public
    {
        require(
            msg.sender == owner,
    ...
    function getListingTokenId(uint256 listingID)
        public
        view
        returns (uint256)
    {
    ...
    function getListingHistoryByTokenId(uint256 tokenId, uint256 index)
        public
        view
        returns (Listing memory)
    {
    ...
    function getListingHistorySize(uint256 tokenId)
        public
        view
        returns (uint256)
    {
    ...
    function getLastListingByTokenId(uint256 tokenId)
        public
        view
        returns (Listing memory)
    {
        return
            getListingHistoryByTokenId(
                tokenId,
                getListingHistorySize(tokenId) - 1
            );
    }
